/**
* @fileoverview Es el main del framework, aqui se incluyen los
* JavaScripts y CSS del proyecto
*
* @author Jose Wilson Capera Castaño
* @author Estefania Alzate Daza
* @version 1.0
*/

KPTF.iniciar();
KPTF.incluirCSS("estilo");
KPTF.incluirCSS("bootstrap.min");
KPTF.incluirJS("cod/clases/aguila");
KPTF.incluirJS("cod/clases/muro");
KPTF.incluirJS("cod/clases/turbo");
KPTF.incluirJS("cod/logica_juego");
KPTF.incluirJS("cod/renderizado");

window.onload = function(){
    cargarLogo();
    //cargarMenuPrincipal();
    //iniciarJuego();
};